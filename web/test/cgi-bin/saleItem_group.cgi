...
use MODULE::StringUtil;
	...
	my $vtype_ti = "";
	if ($FORM{'vtype'} eq 'T') {
		$template       = HTML::Template->new(filename => "./$_CONFIG_template_dir/saleItem_group_title.tmp");
		$vtype_t = '1';
		$vtype  = 'T';
	} elsif ($FORM{'vtype'} eq 'I') {
		$template       = HTML::Template->new(filename => "./$_CONFIG_template_dir/saleItem_group_image.tmp");
		$vtype_i = '1';
		$vtype  = 'I';
	} else {
		$template       = HTML::Template->new(filename => "./$_CONFIG_template_dir/saleItem_group.tmp");
		$vtype  = 'TI';
		$vtype_ti = '1';
	}
	...
	my $gkeyword = $FORM{'gkeyword'};
	$gkeyword = decodUrl($gkeyword);
	my @gkeyword = split(/ |　/, $FORM{'gkeyword'});
	...
	foreach (@group_file) {
		...
		if (sprintf("%02d", $group_line[0]) eq $FORM{'gc'}) {
			$item_group_name = $group_line[1];
			last;
		}
	}
	...
	MAIN:foreach (@list_file) {
		...
		if (($list_line[2] == $FORM{'cc'}) && (($list_line[3] == $FORM{'gc'}))) {
			...
			foreach (@data_file) {
				...
				if ($data_line[0] == $list_line[0]) {
					unless (&isSerachCorrespond(\@data_line, \@gkeyword)) {
						next MAIN;
					}
...
#	$FORM{'vnum'} = 1 unless ($FORM{'vnum'});
	...
	($paging_back, $paging_area, $paging_next) = &pagingOut($paging_back, $paging_next, $FORM{'vnum'}, $total_page, $top_page, $last_page, $FORM{'cc'}, $FORM{'gc'}, '', '', '', $gkeyword, $FORM{'stype'}, $FORM{'vtype'}, 'gc');
...
#		}
		...
		if ($FORM{'vtype'} eq 'T') {
			$hash2 = {
				...
				saleItem_detailInfo_url => 'saleItem_detailInfo.cgi?cc=' . $FORM{'cc'} . '&gc=' . $FORM{'gc'} . '&ic=' . $item_cmd_basic_id,
	...
	$template->param(
		...
		gkeyworddec          => MODULE::StringUtil::conversionSpecialChar($FORM{'gkeyword'}),
		gkeyword             => MODULE::StringUtil::conversionSpecialChar($gkeyword),
